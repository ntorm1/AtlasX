...
Result<const char *> deduceDatetimeFormat(String const &datetime) noexcept {
  static const char *formats[] = {"%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M:%S %Z",
																	"%Y-%m-%d %H:%M:%S %z", "%Y-%m-%d %H:%M:%S %Z %z",
																	"%Y-%m-%d %H:%M:%S %z %Z", "%Y-%m-%d", "%Y/%m/%d", "%Y%m%d",
  };
  for (int i =0 ; i < sizeof(formats)/sizeof(formats[0]); i++) {
	  String format = formats[i];
    if (strToEpoch(datetime, format).has_value()) {
			return formats[i];
		}
  }
  return Err("Could not deduce datetime format");
}

//============================================================================
Result<Int64> strToEpoch(const String &date_string,
